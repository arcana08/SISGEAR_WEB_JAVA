...
<tr>
    ...
    <td><i class="fa fa-edit" style="color:green" data-bs-toggle="modal" data-bs-target="#exampleModal1" onclick="rellenarjs('<% out.print(rs.getString(1)); %>','<% out.print(rs.getString(2)); %>','<% out.print(rs.getString(10)); %>','<% out.print(rs.getString(4)); %>','<% out.print(rs.getString(11)); %>','<% out.print(rs.getString(5)); %>','<% out.print(rs.getString(12)); %>','<% out.print(rs.getString(6)); %>','<% out.print(rs.getString(13)); %>','<% out.print(rs.getString(7)); %>','<% out.print(rs.getString(8)); %>')"></i>
        ...
        <button class="btn btn-success btn-add" 
                onclick="window.open('reportesv/Rproyectiles_1.jsp?id=<% out.print(rs.getString(1)); %>', '_blank')">
...
<%
    ...
    try {
        ...
        String query = "SELECT * from articulo where idtipo = '" + tip + "' AND idmodelos = '" + mod + "'";
        ...
        if (rs.next()) {
            ...
            try {
                query = "INSERT INTO articulo(idmodelos, idclasificaciones, idcalibres, idtipo, art_serie, art_cantidad) values ('" + mod + "','" + clas + "', '" + cal + "', '" + tip + "','NO APLICA', '" + nombre + "')";
    ...
    try {
        Statement st = conn.createStatement();
        String query = "SELECT * from articulo where idtipo = '" + tip + "' AND idmodelos = '" + mod + "'and idarticulo!='" + pkid + "'";
        ResultSet rs = st.executeQuery(query);
        ...
        if (rs.next()) {
            ...
            try {
                query = "update articulo set art_cantidad = '"+nombre+"',idclasificaciones = '"+clas+"',idtipo = '"+tip+"',idcalibres = '"+cal+"',idmodelos = '"+mod+"' where idarticulo = '"+pkid+"'";
                int filasAfectadas = st.executeUpdate(query);
    ...
    try {
        ...
        ResultSet rs = null;
        st = conn.createStatement();
        rs = st.executeQuery("SELECT idclasificaciones, cla_nombre from clasificaciones where idcategorias=5;");
        while (rs.next()) {
%>      
<tr>
    <td><% out.print(rs.getString(1)); %></td>
