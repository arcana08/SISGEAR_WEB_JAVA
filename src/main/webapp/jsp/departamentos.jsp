...
<tr>
    ...
    <td><i class="fa fa-edit" style="color:green" data-bs-toggle="modal" data-bs-target="#exampleModal1" onclick="rellenarjs('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>', '<% out.print(rs.getString(3)); %>', '<% out.print(rs.getString(4)); %>')"></i>
        ...
        <button class="btn btn-success btn-add" 
                onclick="window.open('reportesv/Rdepartamentos_1.jsp?id=<% out.print(rs.getString(1)); %>', '_blank')">
...
<%
    ...
    try {
        ...
        String query = "SELECT * from departamentos where dep_nombre = '" + nombre + "' AND idpaises = '" + pais + "'";
        ResultSet rs = st.executeQuery(query);

        if (rs.next()) {
            // El departamento ya existe
            out.println("<div class='alert alert-danger' role='alert'>El departamento ya existe</div>");
        } else {
            try {
                query = "INSERT into departamentos(dep_nombre, idpaises) values ('" + nombre + "', '" + pais + "')";
    ...
    try {
        ...
        if (rs.next()) {
            ...
            try {
                query = "update departamentos set dep_nombre = '"+nombre+"',idpaises = '"+pais+"' where iddepartamentos = '"+pkid+"'";
                int filasAfectadas = st.executeUpdate(query);
    ...
    try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from departamentos where iddepartamentos = '"+pkid+"'");
        //setTimeout(function () {
