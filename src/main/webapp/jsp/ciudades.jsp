...
<tr>
    ...
    <td><i class="fa fa-edit" style="color:green" data-bs-toggle="modal" data-bs-target="#exampleModal1" onclick="rellenarjs('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>', '<% out.print(rs.getString(3)); %>','<% out.print(rs.getString(4)); %>','<% out.print(rs.getString(6)); %>')"></i>
        ...
        <button class="btn btn-success btn-add" 
                onclick="window.open('reportesv/Rciudades_1.jsp?id=<% out.print(rs.getString(1)); %>', '_blank')">
...
<%
    ...
    String departamento = request.getParameter("dep").toUpperCase();
    
    try {
        Statement st = conn.createStatement();
        String query = "SELECT * from ciudades where ciu_nombre = '"+nombre+"' and iddepartamentos = '"+departamento+"'";
        ...
        if (rs.next()) {
            ...
            try {
                query = "insert into ciudades(ciu_nombre, iddepartamentos) values ('"+nombre+"', '"+departamento+"')";
    ...
    try {
        ...
        if (rs.next()) {
            ...
            try {
                query = "update ciudades set ciu_nombre = '"+nombre+"',iddepartamentos = '"+departamento+"' where idciudades = '"+pkid+"'";
                int filasAfectadas = st.executeUpdate(query);
    ...
    try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from ciudades where idciudades = '"+pkid+"'");
        //setTimeout(function () {
    ...
    try {
        ...
        ResultSet rs = null;
        st = conn.createStatement();
        rs = st.executeQuery("SELECT d.iddepartamentos, d.dep_nombre,d.idpaises,p.pai_nombre FROM departamentos d, paises p where d.idpaises=p.idpaises;");
        while (rs.next()) {
%>
<tr>
    <td><% out.print(rs.getString(1)); %></td>
