...
if (request.getParameter("listar").equals("listar")) {
    try {
        ...
        rs = st.executeQuery("SELECT * FROM calibres;");
        while (rs.next()) {
%>
<tr>
    ...
    <td><i class="fa fa-edit" style="color:green" data-bs-toggle="modal" data-bs-target="#exampleModal1" onclick="rellenarjs('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>')"></i>
        ...
        <button class="btn btn-success btn-add" 
                onclick="window.open('reportesv/Rcalibres_1.jsp?id=<% out.print(rs.getString(1)); %>', '_blank')">
...
<%
    ...
    try {
        ...
        String query = "SELECT * from calibres where cal_nombre = '"+nombre+"'";
        ...
        if (rs.next()) {
            ...
            try {
                query="insert into calibres(cal_nombre)values('"+nombre+"')";
    ...
    try {
        Statement st = conn.createStatement();
        String query = "SELECT * from calibres where cal_nombre = '"+nombre+"' and idcalibres!='"+pkid+"'";
        ResultSet rs = st.executeQuery(query);
        ...
        if (rs.next()) {
            ...
            try {
                query="update calibres set cal_nombre = '"+nombre+"' where idcalibres = '"+pkid+"'";
                int filasAfectadas = st.executeUpdate(query);
    ...
    try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from calibres where idcalibres = '"+pkid+"'");
        //setTimeout(function () {
