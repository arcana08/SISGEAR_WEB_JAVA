...
<tr>
    ...
    <td><i class="fa fa-edit" style="color:green" data-bs-toggle="modal" data-bs-target="#exampleModal1" onclick="rellenarjs('<% out.print(rs.getString(1)); %>', '<% out.print(rs.getString(2)); %>')"></i>
        ...
        <button class="btn btn-success btn-add" 
                onclick="window.open('reportesv/Rroles_1.jsp?id=<% out.print(rs.getString(1)); %>', '_blank')">
...
<%
    ...
    try {
        ...
        String query = "SELECT * from roles where rol_nombre = '"+nombre+"'";
        ...
        if (rs.next()) {
            ...
            try {
                query="insert into roles(rol_nombre)values('"+nombre+"')";
    ...
    try {
        Statement st = conn.createStatement();
        String query = "SELECT * from roles where rol_nombre = '"+nombre+"' and idroles!='"+pkid+"'";
        ResultSet rs = st.executeQuery(query);
        ...
        if (rs.next()) {
            ...
            try {
                query="update roles set rol_nombre = '"+nombre+"' where idroles = '"+pkid+"'";
                int filasAfectadas = st.executeUpdate(query);
    ...
    try {
        Statement st = null;
        st = conn.createStatement();
        st.executeUpdate("delete from roles where idroles = '"+pkid+"'");
        //setTimeout(function () {
